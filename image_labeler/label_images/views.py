...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        if errors:
            return render(
                request, "auth/register.html", {"errors": errors, "username": username}
            )
...
def view_batch_labels(request):
    ...
    batch_of_assets_response = api_get(
...
def view_labels(request):
    ...
    dark_ratios = (
        pd.DataFrame(api_get("get_dark_ratios"))
    ...
    labeled_assets = pd.DataFrame(
        api_get("get_assets_w_rule_labels", {"task_type": task_type})
    )
...
def view_prediction_labels(request):
    ...
    task_by_rule_options = pd.DataFrame(rules_resp["labelling_rules"]).filter(
...
def view_model_results(request):
    ...
    model_results = pd.DataFrame(api_get("get_model_results")["model_results"]).merge(
...
def view_rough_fill(request):
    ...
    rough_fill_scores = pd.DataFrame(
        api_get("get_rough_fill_scores")["rough_fill_scores"]
    )
        ...
        if not response.text or not response.text.strip():
            ...
            try:
                ...
                if not search_results or len(search_results) == 0:
                    ...
                    get_search_term_url = (
                        "https://backend-python-nupj.onrender.com/get_search_term/"
                    )
